...
3. merchant: 가맹점 명을 추출하되, 결제 출처(웹크롤링인지 SMS인지 등)나 괄호 정보가 있다면 이를 유지하여 출처를 명확히 하세요. (예: "네이버페이(배달의민족/SMS)", "네이버페이(웹)") 네이버페이 단독결제의 경우 자산이동인지 실제 소비인지 알 수 있도록 품목을 최대한 유추하세요.
